export interface ManeuverDef {
  ...
  name: string
  desc: string
  /** What the Superiority Die contributes — 'weapon' means it adds to weapon damage (same type as weapon) */
  dmgType: 'weapon' | 'to hit' | 'AC' | 'reduction' | 'temp HP' | 'ally'
}
...
export const MANEUVER_PROGRESSION: { level: number; total: number }[] = [
  { level: 3, total: 3 },
  { level: 7, total: 5 },
  { level: 10, total: 7 },
  { level: 15, total: 9 },
...
export function maneuversKnown(level: number): number {
  let n = 0
  for (const { level: l, total } of MANEUVER_PROGRESSION) {
    if (level >= l) n = total
  }
  return n
}
